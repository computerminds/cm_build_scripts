...
def update_redmine(db_password = None, redmine_host = None, release_tag = None, invalidate_sessions = False):
    ...
    fabric.run("cd /var/www && git clone --reference /var/www/redmine/.git git@github.com:computerminds/redmine.git redmine-%s && cd redmine-%s && git repack -a && rm -f .git/objects/info/alternates && git checkout %s && cd .." % (release_tag, release_tag, release_tag), pty=True)
