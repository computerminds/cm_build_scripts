...
def update_redmine(db_password = None, redmine_host = None, release_tag = None):
    ...
    fabric.run("cd /var/www && git clone --reference /var/www/redmine-staging/.git git@github.com:computerminds/redmine.git %s && cd %s && git repack -a && git checkout %s && cd .." % (build, build, release_tag), pty=True)
